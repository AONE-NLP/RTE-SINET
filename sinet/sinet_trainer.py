...
from sinet import models, prediction
from sinet import sampling
from sinet import util
from sinet.entities import Dataset
from sinet.evaluator import Evaluator
from sinet.input_reader import JsonInputReader, BaseInputReader
from sinet.loss import SiNETLoss, Loss
from tqdm import tqdm
from sinet.trainer import BaseTrainer
from thop import profile

SCRIPT_PATH = os.path.dirname(os.path.realpath(__file__))


class SiNETTrainer(BaseTrainer):
    ...
    def train(self, train_path: str, valid_path: str, types_path: str, input_reader_cls: Type[BaseInputReader]):
        ...
        compute_loss = SiNETLoss(rel_criterion, entity_criterion, model, optimizer, scheduler, args.max_grad_norm)
        best_result = 0
    ...
    def _load_model(self, input_reader):
        ...
        config.sinet_version = model_class.VERSION
        model = model_class.from_pretrained(self._args.model_path,
                                            config=config,
                                            # SiNET model parameters
    ...
    def _train_epoch(self, model: torch.nn.Module, compute_loss: Loss, optimizer: Optimizer, dataset: Dataset,
        ...
        for batch in tqdm(data_loader, total=total, desc='Train epoch %s' % epoch):
            ...
            # macs, params = profile(model, inputs=(batch['encodings'], batch['context_masks'],batch['entity_masks'], batch['entity_sizes'],batch['rels'], batch['rel_masks']))
